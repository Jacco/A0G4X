define(["jquery", "THREE", "THREE/TrackballControls", "THREE/ColladaLoader"], function($, THREE, TrackballControls, ColladaLoader) {
	...
	return function(element) {
		...
		var grid = new THREE.GridHelper(20000, 250);
		...
		*/
		...
		var createTrajectory = function(plot_type) {
			...
			trajectory.mesh = new THREE.Line(trajectory.geometry, undefined, THREE.LinePieces);
		...
		var covariance2d = new THREE.Mesh(new THREE.RingGeometry(490, 510, 16, 16), new THREE.MeshBasicMaterial());
		...
		return {
			...
			setCovariance2D: function(rotation, scale) {
				...
				//));
				//console.log(scale)
				covariance2d.scale.set(Math.abs(scale[0]) * 0.01, Math.abs(scale[1]) * 0.01, 1);
			},
